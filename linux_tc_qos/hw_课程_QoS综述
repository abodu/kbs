...
1. 报文解析：设备会截取报文的前128字节字段，解析各层次的报文头。比如：简单流分类的字段，802.1p/DSCP/MPLS EXP字段等。复杂流分类的字段，五元组、基于二层封装的协议字以及ACL、ACL6匹配的字段等。
...
5. Merter&Counter（度量&统计）：通过令牌桶机制对网络流量进行度量，并对度量结果进行统计。
...
10. Engress ACL：出端口方向ACL查找，此处可以根据对数据流的分类结果应用报文过滤、报文重标记等策略。
11. 报文解析：设备同样截取报文的前128 字节字段，解析各层次的报文头，此处获取的为经过处理的设备内部优先级和丢弃优先级。
